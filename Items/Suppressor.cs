...
namespace EpikV2.Items {
    ...
    public class Suppressor : ModItem, ICustomDrawItem  {
        ...
        public override void HoldItem(Player player) {
            if(split>0)split-=8-item.useTime;
            if(split<0)split = 0;
            if(delay > 0) {
                delay--;
            }
            ...
            if(player.altFunctionUse == 2) {
                if(player.controlUseTile) {
                    ...
                    for(int i = 7; i > 0; i--) {
			            ...
			            Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<SuppressorShot>(), (int)(damage*1.5), knockBack, player.whoAmI);
                    }
                    player.altFunctionUse = 0;
                    split = 180;
                    delay = 180/(8-item.useTime)+(item.useTime*3);
        ...
        public void DrawInHand(Texture2D itemTexture, PlayerDrawInfo drawInfo, Vector2 itemCenter, Vector4 lightColor, Vector2 drawOrigin) {
            ...
            pos+=(unit*splitValue*splitMult)-(splitShake*1.5f);
            ...
            value.shader = 84;
            Main.playerDrawData.Add(value);
            value = new DrawData(bottomTexture, pos-(splitShake*1.5f)+(unit.RotatedBy(1f*drawPlayer.direction)*splitValue*splitMult+splitShake.RotatedByRandom(Math.PI)), new Rectangle(0, 0, itemTexture.Width, itemTexture.Height), item.GetAlpha(new Color(lightColor.X, lightColor.Y, lightColor.Z, lightColor.W)), itemRotation, drawOrigin, item.scale, drawInfo.spriteEffects, 0);
            value.shader = 84;
            Main.playerDrawData.Add(value);
            value = new DrawData(topTexture, pos-(splitShake*1.5f)+(unit.RotatedBy(-1f*drawPlayer.direction)*splitValue*splitMult+splitShake.RotatedByRandom(Math.PI)), new Rectangle(0, 0, itemTexture.Width, itemTexture.Height), item.GetAlpha(new Color(lightColor.X, lightColor.Y, lightColor.Z, lightColor.W)), itemRotation, drawOrigin, item.scale, drawInfo.spriteEffects, 0);
            value.shader = 84;
            Main.playerDrawData.Add(value);
        }
    }
